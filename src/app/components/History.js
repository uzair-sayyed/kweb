...
const History = () => {
  ...
  return (
    <div className='relative  mx-auto py-12 history'>
      <h2 className='text-[35px] font-medium mb-12 max-w-[1400px] mx-auto'>
        ...
        {timelineData.map((item, index) => (
          <div key={index} className='relative'>
            <div className='grid rounded-lg'>
              <h3 className='text-xl font-bold text-[#0D63AD] mb-2 p-4'>
                {item.year}
              </h3>
              <div className='relative w-full'>
                <div className='absolute top-1/2 w-full h-0.5 bg-[#0000004F] transform -translate-y-1/2'></div>
                <div className="border border-[#0D63AD] rounded-full p-[5px] absolute top-1/2 left-[8%] transform -translate-x-1/2 -translate-y-1/2">
                  <div className='w-[12px] h-[12px] bg-[#0D63AD] rounded-full'></div>
                </div>
              </div>
              <p className='text-gray-600 mt-6 p-4'>{item.description}</p>
            </div>
          </div>
        ))}
      </Slider>
...
const NextArrow = ({ onClick }) => (
  <div
    className='slick-arrow slick-next right-[20%] top-[-75px] rounded-full p-3 cursor-pointer text-white'
    onClick={onClick}
  >
